...
\fix{map(f, map(g, lst))} then a compiler pass can transform this
...
\fix{f}. Considering function \fix{f} is isolated and map limit
data dependency among different output, an analysis can be done for
function \fix{f} to track data sharing among multiple outputs. Here,
